-- | Some Metal platforms have a SIMD/warp/wavefront-based execution
...
-- counts.
module Futhark.CodeGen.Metal.Heuristics
  ( SizeHeuristic (..),
    DeviceType (..),
    WhichSize (..),
    DeviceInfo (..),
...
-- | The type of Metal device that this heuristic applies to.
...
sizeHeuristicsTable =
  [ SizeHeuristic "Apple M1 Metal" DeviceGPU LockstepWidth 32,
