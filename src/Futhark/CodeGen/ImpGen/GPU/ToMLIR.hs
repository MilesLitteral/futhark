...
module Futhark.CodeGen.ImpGen.GPU.ToMLIR
  ( kernelsToMLIR,
  )
where
...
import Futhark.CodeGen.ImpCode.MLIR hiding (Program)
import qualified Futhark.CodeGen.ImpCode.MLIR as ImpMLIR
...
onKernel :: KernelTarget -> Kernel -> OnKernelM MLIR
...
useAsParam (ScalarUse name pt) = do
  let name_bits = zEncodeString (pretty name) <> "_bits"
      ctp = case pt of
        -- MLIR does not permit bool as a kernel parameter type.
...
genMetalPrelude ts =
  [untrimming|
// Clang-based MLIR implementations need this for 'static' to work.
