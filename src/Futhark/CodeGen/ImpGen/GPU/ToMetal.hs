...
module Futhark.CodeGen.ImpGen.GPU.ToMetal
  ( kernelsToMetal,
  )
where
...
-- | Translate a kernels-program to an Metal-program.
translateGPU ::
  KernelTarget ->
  ImpGPU.Program ->
  ImpMetal.Program
...
useAsParam (ScalarUse name pt) = do
  let name_bits = zEncodeString (pretty name) <> "_bits"
      ctp = case pt of
        -- Metal does not permit bool as a kernel parameter type.
...
kernel void dummy_kernel(device const float* inA,
  ...
  where
    enable_f64
      | FloatType Float64 `S.member` ts =
        [untrimming|
         #pragma METAL EXTENSION cl_khr_fp64 : enable
