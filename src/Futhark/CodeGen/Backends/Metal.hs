...
module Futhark.CodeGen.Backends.Metal
  ( compileProg,
    GC.CParts (..),
    GC.asLibrary,
    GC.asExecutable,
    ...
    operations =
      GC.defaultOperations
        { GC.opsWriteScalar = writeMetalScalar,
          ...
          GC.opsCritical =
            ( [C.citems|METAL_SUCCEED_FATAL(cuCtxPushCurrent(ctx->metal.cu_ctx));|],
              [C.citems|METAL_SUCCEED_FATAL(cuCtxPopCurrent(&ctx->metal.cu_ctx));|]
            )
        }
    metal_includes =
      [untrimming|
...
allocateMetalBuffer mem size tag "device" =
  GC.stm [C.cstm|METAL_SUCCEED_OR_RETURN(metal_alloc(&ctx->metal, (size_t)$exp:size, $exp:tag, &$exp:mem));|]
allocateMetalBuffer _ _ _ space =
  error $ "Cannot allocate in '" ++ space ++ "' memory space."

deallocateMetalBuffer :: GC.Deallocate OpenCL ()
deallocateMetalBuffer mem tag "device" =
  GC.stm [C.cstm|METAL_SUCCEED_OR_RETURN(metal_free(&ctx->metal, $exp:mem, $exp:tag));|]
deallocateMetalBuffer _ _ space =
  error $ "Cannot deallocate in '" ++ space ++ "' memory space."

copyMetalMemory :: GC.Copy OpenCL ()
...
staticMetalArray name "device" t vs = do
  ...
  GC.atInit
    ...
    ctx->$id:name.size = 0;
    METAL_SUCCEED_FATAL(cuMemAlloc(&ctx->$id:name.mem,
                            ($int:num_elems > 0 ? $int:num_elems : 1)*sizeof($ty:ct)));
    if ($int:num_elems > 0) {
      METAL_SUCCEED_FATAL(cuMemcpyHtoD(ctx->$id:name.mem, $id:name_realtype,
...
callKernel (LaunchKernel safety kernel_name args num_blocks block_size) = do
  ...
  GC.stm
    ...
    if ($exp:sizes_nonzero) {
      ...
      $items:aft
      if (ctx->debugging) {
        METAL_SUCCEED_FATAL(cuCtxSynchronize());
